...
function WashDialog({ open, setOpen, flows, products, submit, busy }: DialogProps & { flows: WashFlow[]; products: Product[] }) {
  const [step, setStep] = useState(1), [flowId, setFlowId] = useState<number | null>(null), [usages, setUsages] = useState<Record<number, number>>({}), [mixes, setMixes] = useState<Record<number, WashMix>>({}), [date, setDate] = useState(new Date().toISOString().slice(0, 10)), [note, setNote] = useState("");
  ...
  function choose(flow: WashFlow) {
    ...
    setFlowId(flow.id); setUsages(nextUsages); setMixes(nextMixes);
  }
  async function save() {
    if (!flowId || !selectedFlow) return;
    const list = selectedFlow.items.map((item) => {
