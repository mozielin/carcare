...
import { AlertTriangle, Archive, ArrowDown, ArrowUp, Beaker, CalendarDays, Download, Droplets, History, Menu, PackagePlus, Pencil, Plus, Sparkles, Trash2, Upload } from "lucide-react";
...
type DilutionProfile = { id: number; productId: number; name: string; ratio: number; defaultWater: number };
type DilutionDraft = { name: string; ratio: string; defaultWater: number };
type Product = { id: number; brand: string; name: string; category: string; unit: string; packageSize: number; remaining: number; lowThreshold: number; phType: "酸性" | "中性" | "鹼性"; affectedFlowNames: string | null; dilutions: DilutionProfile[] };
...
function AddProductDialog({
  ...
  const [packageSize, setPackageSize] = useState(""),
    ...
    [dilutions, setDilutions] = useState<DilutionDraft[]>([]);
  ...
  async function save(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await submit(
        "addProduct",
        { ...Object.fromEntries(new FormData(event.currentTarget)), dilutions: cleanDilutionDrafts(dilutions) },
      );
      setPackageSize("");
      setRemaining("");
      setLowThreshold("");
    ...
    >
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto rounded-3xl">
        ...
        <form onSubmit={save} aria-busy={locked} className="grid gap-4">
          ...
          {!consumable && <DilutionEditor value={dilutions} onChange={setDilutions} disabled={locked} />}
          <DialogFooter>
            <Button
              disabled={locked}
              type="submit"
...
function EditProductDialog({ product, setProduct, submit, busy, brands }: { product: Product | null; setProduct: (v: Product | null) => void; submit: Submit; busy: boolean; brands: string[] }) { return <Dialog open={!!product} onOpenChange={(v) => !v && setProduct(null)}><DialogContent className="max-h-[90vh] overflow-y-auto rounded-3xl"><DialogHeader><DialogTitle>編輯洗車用品</DialogTitle><DialogDescription>修改用品資料；流程關聯與過去洗車紀錄會保留。</DialogDescription></DialogHeader>{product && <EditProductForm key={product.id} product={product} setProduct={setProduct} submit={submit} busy={busy} brands={brands} />}</DialogContent></Dialog>; }
...
function EditProductForm({ product, setProduct, submit, busy, brands }: { product: Product; setProduct: (v: Product | null) => void; submit: Submit; busy: boolean; brands: string[] }) {
  ...
  const [dilutions, setDilutions] = useState<DilutionDraft[]>(product.dilutions.map((item) => ({ name: item.name, ratio: String(item.ratio), defaultWater: item.defaultWater })));
  const consumable = category === "耗材";
  async function save(form: FormData) {
    try {
      await submit("updateProduct", { productId: product.id, ...Object.fromEntries(form), dilutions: consumable ? [] : cleanDilutionDrafts(dilutions) });
      setProduct(null);
      toast.success("用品資料已更新");
    } catch (e) { toast.error(msg(e)); }
  }
  return <form action={save} className="grid gap-4"><BrandField brands={brands} initialBrand={product.brand} /><Field label="用品名稱"><input className={inputClass} name="name" defaultValue={product.name} required /></Field><div className="grid grid-cols-3 gap-3"><Field label="用途分類"><Select name="category" value={category} onValueChange={setCategory}><SelectTrigger className={`${inputClass} w-full`}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="預洗">預洗</SelectItem><SelectItem value="正洗">正洗</SelectItem><SelectItem value="保養">保養</SelectItem><SelectItem value="玻璃">玻璃</SelectItem><SelectItem value="耗材">耗材</SelectItem><SelectItem value="內裝">內裝</SelectItem><SelectItem value="其他">其他</SelectItem></SelectContent></Select></Field><Field label="pH 分類"><select className={inputClass} name="phType" defaultValue={product.phType} disabled={consumable}><option>酸性</option><option>中性</option><option>鹼性</option></select></Field><Field label="單位">{consumable ? <input className={inputClass} name="unit" value="次" readOnly /> : <select className={inputClass} name="unit" defaultValue={product.unit}><option>ml</option><option>g</option><option>顆</option><option>片</option></select>}</Field></div><div className="grid grid-cols-3 gap-3"><Field label={consumable ? "可使用次數" : "瓶身容量"}><NumberInput name="packageSize" defaultValue={product.packageSize} /></Field><Field label={consumable ? "剩餘次數" : "目前剩餘"}><NumberInput name="remaining" defaultValue={product.remaining} /></Field><Field label={consumable ? "更換提醒" : "提醒門檻"}><NumberInput name="lowThreshold" defaultValue={product.lowThreshold} /></Field></div>{!consumable && <DilutionEditor value={dilutions} onChange={setDilutions} disabled={busy} />}<DialogFooter><Button variant="outline" type="button" disabled={busy} onClick={() => setProduct(null)} className="rounded-xl">取消</Button><Button disabled={busy} type="submit" className="rounded-xl bg-cyan-950">{busy ? "儲存中…" : "儲存修改"}</Button></DialogFooter></form>;
}

function DilutionEditor({ value, onChange, disabled }: { value: DilutionDraft[]; onChange: (value: DilutionDraft[]) => void; disabled: boolean }) {
  function update(index: number, next: Partial<DilutionDraft>) { onChange(value.map((item, itemIndex) => itemIndex === index ? { ...item, ...next } : item)); }
  return <section className="rounded-2xl border border-cyan-100 bg-cyan-50/60 p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-semibold text-cyan-950">稀釋比例（選填）</p><p className="mt-1 text-xs text-cyan-800/70">比例定義為藥劑：水＝1:X，可設定多組。</p></div><Button type="button" variant="outline" disabled={disabled} onClick={() => onChange([...value, { name: "", ratio: "", defaultWater: 2000 }])} className="h-9 shrink-0 rounded-xl bg-white"><Plus className="size-4" />新增比例</Button></div>{value.length === 0 ? <p className="mt-4 rounded-xl bg-white/70 px-3 py-4 text-center text-xs text-slate-500">尚未設定；洗車時仍可直接輸入使用量。</p> : <div className="mt-4 grid gap-3">{value.map((item, index) => <div key={index} className="rounded-2xl border border-cyan-100 bg-white p-3"><div className="grid grid-cols-[minmax(0,1fr)_110px_auto] items-end gap-2"><Field label="名稱"><input className={inputClass} value={item.name} disabled={disabled} onChange={(event) => update(index, { name: event.target.value })} placeholder="一般／加強" /></Field><Field label="比例"><div className="relative"><span className="absolute left-3 top-3 text-sm text-slate-500">1:</span><input className={`${inputClass} pl-8`} type="number" inputMode="numeric" min="1" step="1" value={item.ratio} disabled={disabled} onChange={(event) => update(index, { ratio: event.target.value })} aria-label={`${item.name || `第 ${index + 1} 組`}稀釋比例`} /></div></Field><button type="button" disabled={disabled} onClick={() => onChange(value.filter((_, itemIndex) => itemIndex !== index))} className="grid size-11 place-items-center rounded-xl text-slate-400 hover:bg-red-50 hover:text-red-600 disabled:opacity-40" aria-label={`刪除第 ${index + 1} 組比例`}><Trash2 className="size-4" /></button></div><div className="mt-4"><div className="mb-3 flex items-center justify-between text-xs"><span className="font-medium text-slate-600">預設水量</span><strong className="text-cyan-900">{format(item.defaultWater)} ml</strong></div><Slider value={[item.defaultWater]} min={500} max={5000} step={500} disabled={disabled} onValueChange={(values) => update(index, { defaultWater: Number(values[0]) })} aria-label={`${item.name || `第 ${index + 1} 組`}預設水量`} /><div className="mt-2 flex justify-between text-[10px] text-slate-400"><span>500 ml</span><span>5,000 ml</span></div></div></div>)}</div>}</section>;
}

function cleanDilutionDrafts(value: DilutionDraft[]) { return value.filter((item) => item.name.trim() || item.ratio).map((item) => ({ name: item.name.trim(), ratio: Number(item.ratio), defaultWater: item.defaultWater })); }
...
}, [open, flow]); async function save() { const selected = new Set(Object.entries(items).filter(([, amount]) => amount > 0).map(([productId]) => Number(productId))); const ids = [...order.filter((id) => selected.has(id)), ...[...selected].filter((id) => !order.includes(id))]; const list = ids.map((productId) => ({ productId, amount: items[productId] })); try { await submit("saveFlow", { flowId: flow?.id, flowType, name, items: list }); setOpen(false); toast.success("流程設定已儲存"); } catch (e) { toast.error(msg(e)); } } return <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-h-[90vh] overflow-y-auto rounded-3xl sm:max-w-xl"><DialogHeader><DialogTitle>{flow ? `編輯 ${flow.name}` : "新增自訂流程"}</DialogTitle><DialogDescription>設定流程名稱、預設藥劑與每次使用劑量；使用上下箭頭調整已選藥劑的順序。</DialogDescription></DialogHeader><div className="grid gap-4"><Field label="流程類型"><select disabled={!!flow && flow.flowType !== "自訂流程"} className={inputClass} value={flowType} onChange={(e) => setFlowType(e.target.value)}><option>2PH</option><option>3PH</option><option>快速保養</option><option>自訂流程</option></select></Field>{flowType === "自訂流程" && <Field label="流程名稱"><input className={inputClass} value={name} onChange={(e) => setName(e.target.value)} placeholder="例如：雨後加強洗" /></Field>}<ProductPicker products={products} values={items} setValues={setItems} order={order} setOrder={setOrder} reorderable /></div><DialogFooter><Button disabled={busy} onClick={save} className="rounded-xl bg-cyan-950">{busy ? "儲存中…" : "儲存流程"}</Button></DialogFooter></DialogContent></Dialog>; }

type WashMix = { profileId: number; water: number };

function WashDialog({ open, setOpen, flows, products, submit, busy }: DialogProps & { flows: WashFlow[]; products: Product[] }) {
  const [step, setStep] = useState(1), [flowId, setFlowId] = useState<number | null>(null), [adHoc, setAdHoc] = useState(false), [customProductIds, setCustomProductIds] = useState<number[]>([]), [usages, setUsages] = useState<Record<number, number>>({}), [mixes, setMixes] = useState<Record<number, WashMix>>({}), [date, setDate] = useState(new Date().toISOString().slice(0, 10)), [note, setNote] = useState("");
  const selectedFlow = flows.find((f) => f.id === flowId);
  const customItems: FlowItem[] = customProductIds.map((productId) => { const product = products.find((candidate) => candidate.id === productId); return { flowId: 0, productId, amount: usages[productId] || 1, productName: product?.name || "", unit: product?.unit || "ml" }; });
  const activeItems = adHoc ? customItems : selectedFlow?.items || [];
  function choose(flow: WashFlow) {
    const nextUsages: Record<number, number> = {}, nextMixes: Record<number, WashMix> = {};
    for (const item of flow.items) {
      const product = products.find((candidate) => candidate.id === item.productId), profile = product?.unit === "ml" ? product.dilutions[0] : undefined;
      if (profile) { nextMixes[item.productId] = { profileId: profile.id, water: profile.defaultWater }; nextUsages[item.productId] = dilutionDose(profile.defaultWater, profile.ratio); }
      else nextUsages[item.productId] = item.amount;
    }
    setAdHoc(false); setCustomProductIds([]); setFlowId(flow.id); setUsages(nextUsages); setMixes(nextMixes);
  }
  function chooseAdHoc() {
    setAdHoc(true); setFlowId(null); setCustomProductIds([]); setUsages({}); setMixes({});
  }
  function toggleCustomProduct(product: Product, enabled: boolean) {
    const nextUsages = { ...usages }, nextMixes = { ...mixes };
    if (enabled) {
      const profile = product.unit === "ml" ? product.dilutions[0] : undefined;
      setCustomProductIds([...customProductIds, product.id]);
      if (profile) { nextMixes[product.id] = { profileId: profile.id, water: profile.defaultWater }; nextUsages[product.id] = dilutionDose(profile.defaultWater, profile.ratio); }
      else nextUsages[product.id] = 1;
    } else {
      setCustomProductIds(customProductIds.filter((id) => id !== product.id));
      delete nextUsages[product.id]; delete nextMixes[product.id];
    }
    setUsages(nextUsages); setMixes(nextMixes);
  }
  async function save() {
    if ((!adHoc && (!flowId || !selectedFlow)) || !activeItems.length) return;
    const list = activeItems.map((item) => {
      const product = products.find((candidate) => candidate.id === item.productId), mix = mixes[item.productId], profile = product?.dilutions.find((candidate) => candidate.id === mix?.profileId);
      return { productId: item.productId, amount: profile && mix ? dilutionDose(mix.water, profile.ratio) : usages[item.productId] };
    }).filter((item) => item.amount > 0);
    try { await submit("logWash", { flowId, adHoc, washedAt: date, note, usages: list }); setOpen(false); setStep(1); setFlowId(null); setAdHoc(false); setCustomProductIds([]); setUsages({}); setMixes({}); setNote(""); toast.success("洗車完成，庫存已扣除"); } catch (e) { toast.error(msg(e)); }
  }
  return <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) setStep(1); }}><DialogContent className="max-h-[90vh] overflow-y-auto rounded-3xl sm:max-w-xl"><DialogHeader><DialogTitle>記錄本次洗車</DialogTitle><DialogDescription>步驟 {step} / 2　{step === 1 ? "選擇這次洗車流程" : "確認用品、水量與用量"}</DialogDescription></DialogHeader>{step === 1 ? <div className="grid gap-3">{flows.map((flow) => <button key={flow.id} disabled={!flow.items.length} onClick={() => choose(flow)} className={`rounded-2xl border p-4 text-left transition disabled:cursor-not-allowed disabled:opacity-45 ${!adHoc && flowId === flow.id ? "border-cyan-600 bg-cyan-50 ring-4 ring-cyan-600/10" : "border-slate-200 hover:border-cyan-300"}`}><div className="flex justify-between"><strong>{flow.name}</strong><span className="text-xs text-slate-400">{flow.items.length} 項藥劑</span></div><ol className="mt-2 list-inside list-decimal space-y-0.5 text-xs text-slate-500">{flow.items.length ? flow.items.map((i) => <li key={i.productId}>{i.productName}</li>) : <li className="list-none">請先到流程管理設定藥劑</li>}</ol></button>)}<button type="button" onClick={chooseAdHoc} className={`rounded-2xl border p-4 text-left transition ${adHoc ? "border-cyan-600 bg-cyan-50 ring-4 ring-cyan-600/10" : "border-dashed border-cyan-300 bg-cyan-50/40 hover:border-cyan-500"}`}><div className="flex items-center justify-between gap-3"><div><strong>自訂</strong><p className="mt-1 text-xs text-slate-500">這次自由挑選用品，不變更流程範本</p></div><Plus className="size-5 text-cyan-700" /></div></button><DialogFooter><Button disabled={!flowId && !adHoc} onClick={() => setStep(2)} className="rounded-xl bg-cyan-950">下一步：確認用量</Button></DialogFooter></div> : <div className="grid gap-4"><div className="rounded-2xl bg-cyan-950 p-4 text-white"><p className="text-xs text-cyan-200">本次流程</p><p className="mt-1 font-semibold">{adHoc ? "自訂" : selectedFlow?.name}</p></div><Field label="洗車日期"><input className={inputClass} type="date" value={date} onChange={(e) => setDate(e.target.value)} /></Field>{adHoc && <section className="rounded-2xl border border-slate-200 bg-white p-4"><p className="text-sm font-semibold text-slate-900">選擇本次使用的用品</p><p className="mt-1 text-xs text-slate-500">可自由勾選，選好後在下方設定每項用量。</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{products.map((product) => { const checked = customProductIds.includes(product.id); return <label key={product.id} className={`flex cursor-pointer items-start gap-3 rounded-xl border p-3 transition ${checked ? "border-cyan-400 bg-cyan-50" : "border-slate-200 hover:border-cyan-300"}`}><Checkbox checked={checked} onCheckedChange={(value) => toggleCustomProduct(product, !!value)} /><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold text-slate-900">{product.name}</span><span className="mt-0.5 block truncate text-[11px] text-slate-500">{product.brand || "未設定品牌"}</span><span className="mt-2 flex flex-wrap items-center gap-1.5"><span className="rounded-full bg-slate-200 px-2 py-0.5 text-[10px] font-medium text-slate-700">{product.category}</span>{product.category !== "耗材" && <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${phClass(product.phType)}`}>{product.phType}</span>}<span className="text-[10px] text-slate-500">庫存 {format(product.remaining)} {product.unit}</span></span></span></label>; })}</div></section>}{adHoc && !activeItems.length ? <p className="rounded-2xl bg-amber-50 px-4 py-3 text-sm text-amber-800">請先勾選至少一項用品。</p> : <WashUsagePicker items={activeItems} products={products} values={usages} setValues={setUsages} mixes={mixes} setMixes={setMixes} />}<Field label="備註（選填）"><input className={inputClass} value={note} onChange={(e) => setNote(e.target.value)} /></Field><DialogFooter><Button variant="outline" disabled={busy} onClick={() => setStep(1)} className="rounded-xl">上一步</Button><Button disabled={busy || !activeItems.length} onClick={save} className="rounded-xl bg-cyan-950">{busy ? "扣除中…" : "確認並扣除庫存"}</Button></DialogFooter></div>}</DialogContent></Dialog>;
}

function WashUsagePicker({ items, products, values, setValues, mixes, setMixes }: { items: FlowItem[]; products: Product[]; values: Record<number, number>; setValues: (value: Record<number, number>) => void; mixes: Record<number, WashMix>; setMixes: (value: Record<number, WashMix>) => void }) {
  return <div className="grid gap-3">{items.map((item) => { const product = products.find((candidate) => candidate.id === item.productId); if (!product) return null; const mix = mixes[product.id], profile = product.dilutions.find((candidate) => candidate.id === mix?.profileId), calculated = profile && mix ? dilutionDose(mix.water, profile.ratio) : values[product.id]; return <article key={product.id} className="rounded-2xl border border-cyan-300 bg-cyan-50 p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-semibold">{product.name}</p><p className="mt-0.5 truncate text-[11px] text-slate-500">品牌：{product.brand || "未設定品牌"}</p><div className="mt-2 flex flex-wrap gap-1.5"><span className="rounded-full bg-slate-200 px-2 py-0.5 text-[11px]">{product.category}</span>{product.category !== "耗材" && <span className={`rounded-full px-2 py-0.5 text-[11px] ${phClass(product.phType)}`}>{product.phType}</span>}</div></div><div className="text-right"><p className="text-[11px] text-slate-500">庫存</p><p className="text-sm font-medium tabular-nums">{format(product.remaining)} {product.unit}</p></div></div>{product.unit === "ml" && product.dilutions.length > 0 ? <div className="mt-4 rounded-2xl bg-white p-3"><div className="grid grid-cols-2 gap-3"><Field label="稀釋模式"><select className={inputClass} value={mix?.profileId || 0} onChange={(event) => { const profileId = Number(event.target.value), nextProfile = product.dilutions.find((candidate) => candidate.id === profileId); if (!nextProfile) { setMixes({ ...mixes, [product.id]: { profileId: 0, water: 2000 } }); return; } setMixes({ ...mixes, [product.id]: { profileId, water: nextProfile.defaultWater } }); setValues({ ...values, [product.id]: dilutionDose(nextProfile.defaultWater, nextProfile.ratio) }); }}><option value={0}>直接輸入</option>{product.dilutions.map((option) => <option key={option.id} value={option.id}>{option.name} 1:{option.ratio}</option>)}</select></Field>{profile ? <div className="rounded-xl bg-cyan-950 px-3 py-2 text-white"><p className="text-[10px] text-cyan-200">應加藥劑</p><p className="mt-1 text-lg font-semibold tabular-nums">{format(calculated)} ml</p></div> : <Field label="藥劑用量"><div className="relative"><input className={`${inputClass} pr-9`} type="number" inputMode="numeric" min="1" step="1" value={values[product.id] > 0 ? values[product.id] : ""} onChange={(event) => setValues({ ...values, [product.id]: Number(event.target.value) })} /><span className="absolute right-3 top-3 text-xs text-slate-500">ml</span></div></Field>}</div>{profile && mix && <div className="mt-4"><div className="mb-3 flex items-center justify-between text-xs"><span className="font-medium text-slate-600">水量（每格 500 ml）</span><strong className="text-cyan-900">{format(mix.water)} ml</strong></div><Slider value={[mix.water]} min={500} max={5000} step={500} onValueChange={(sliderValue) => { const water = Number(sliderValue[0]); setMixes({ ...mixes, [product.id]: { ...mix, water } }); setValues({ ...values, [product.id]: dilutionDose(water, profile.ratio) }); }} aria-label={`${product.name} 水量`} /><div className="mt-2 flex justify-between text-[10px] text-slate-400"><span>500 ml</span><span>5,000 ml</span></div><p className="mt-3 text-xs text-slate-500">藥劑：水＝1:{profile.ratio}，{format(mix.water)} ml 水需加入 {format(calculated)} ml 藥劑。</p></div>}</div> : <Field label={product.category === "耗材" ? "本次使用次數" : "本次使用量"}><div className="relative mt-3"><input className={`${inputClass} pr-9`} type="number" inputMode="numeric" min="1" step="1" value={values[product.id] > 0 ? values[product.id] : ""} onChange={(event) => setValues({ ...values, [product.id]: Number(event.target.value) })} /><span className="absolute right-3 top-3 text-xs text-slate-500">{product.unit}</span></div></Field>}</article>; })}</div>;
}

function dilutionDose(water: number, ratio: number) { return Math.max(1, Math.round(water / ratio)); }

function ProductPicker({ products, values, setValues, locked = false, order = [], setOrder, reorderable = false }: { products: Product[]; values: Record<number, number>; setValues: (v: Record<number, number>) => void; locked?: boolean; order?: number[]; setOrder?: (v: number[]) => void; reorderable?: boolean }) { const selectedIds = order.filter((id) => values[id] !== undefined); const sorted = reorderable ? [...selectedIds.map((id) => products.find((p) => p.id === id)).filter((p): p is Product => !!p), ...products.filter((p) => values[p.id] === undefined)] : products; function toggle(id: number, enabled: boolean) { const next = { ...values }; if (enabled) { next[id] = 1; setOrder?.([...selectedIds, id]); } else { delete next[id]; setOrder?.(selectedIds.filter((item) => item !== id)); } setValues(next); } function moveBy(id: number, offset: -1 | 1) { if (!setOrder) return; const index = selectedIds.indexOf(id), target = index + offset; if (index < 0 || target < 0 || target >= selectedIds.length) return; const next = [...selectedIds]; [next[index], next[target]] = [next[target], next[index]]; setOrder(next); } return <div className="grid gap-2">{sorted.map((p) => { const checked = values[p.id] !== undefined, position = selectedIds.indexOf(p.id); return <div key={p.id} className={`grid grid-cols-[auto_minmax(0,1fr)_130px] items-center gap-3 rounded-2xl border p-3 ${checked ? "border-cyan-400 bg-cyan-50" : "border-slate-200 bg-white"}`}>{reorderable ? <div className="flex items-center gap-1.5">{checked && <div className="grid gap-1"><button type="button" disabled={position === 0} onClick={() => moveBy(p.id, -1)} className="grid size-7 place-items-center rounded-lg bg-white text-cyan-900 shadow-sm transition hover:bg-cyan-100 disabled:cursor-not-allowed disabled:opacity-25" title="上移一格" aria-label={`將 ${p.name} 上移一格`}><ArrowUp className="size-4" /></button><button type="button" disabled={position === selectedIds.length - 1} onClick={() => moveBy(p.id, 1)} className="grid size-7 place-items-center rounded-lg bg-white text-cyan-900 shadow-sm transition hover:bg-cyan-100 disabled:cursor-not-allowed disabled:opacity-25" title="下移一格" aria-label={`將 ${p.name} 下移一格`}><ArrowDown className="size-4" /></button></div>}<div className="grid gap-1.5"><span className={`grid size-5 place-items-center rounded-full text-[11px] font-bold ${checked ? "bg-cyan-800 text-white" : "bg-slate-200 text-slate-500"}`}>{checked ? position + 1 : "—"}</span><Checkbox checked={checked} onCheckedChange={(v) => toggle(p.id, !!v)} /></div></div> : locked ? <Beaker className="size-4 text-cyan-700" /> : <Checkbox checked={checked} onCheckedChange={(v) => toggle(p.id, !!v)} />}<div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-900">{p.name}</p><p className="mt-0.5 truncate text-[11px] font-medium text-slate-500">品牌：{p.brand || "未設定品牌"}</p><div className="mt-1 flex flex-wrap items-center gap-1.5"><span className="rounded-full bg-slate-200 px-2 py-0.5 text-[11px] font-medium text-slate-700">{p.category}</span>{p.category !== "耗材" && <span className={`rounded-full px-2 py-0.5 text-[11px] font-medium ${phClass(p.phType)}`}>{p.phType}</span>}<span className="text-xs text-slate-500">庫存 {format(p.remaining)} {p.unit}</span></div></div><label className="grid gap-1"><span className="text-xs font-medium text-slate-600">每次使用</span><div className="relative"><input aria-label={`${p.name} 每次使用`} className={`${inputClass} pr-9`} disabled={!checked} type="number" inputMode="numeric" min="1" max={p.remaining} step="1" value={checked && values[p.id] > 0 ? values[p.id] : ""} onChange={(e) => setValues({ ...values, [p.id]: Number(e.target.value) })} /><span className="absolute right-3 top-3 text-xs font-medium text-slate-500">{p.unit}</span></div></label></div>; })}</div>; }
...
type BackupFile = { format: string; version: number; exportedAt?: string; data: { products?: unknown[]; dilutionProfiles?: unknown[]; flows?: unknown[]; flowItems?: unknown[]; washes?: unknown[]; washUsages?: unknown[]; restocks?: unknown[] } };
